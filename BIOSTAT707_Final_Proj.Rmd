...
density_plot <- function(col,P){
  ggplot(cardio_train_0, aes(x = col, fill = cardio))+
    geom_density(alpha = 0.5)+
    theme(legend.position = "bottom")+
    scale_fill_manual(values=c("blue", "gold", "#56B4E9"))+
...
ap_hi_plot <- density_plot(cardio_train_0$ap_hi, "ap_hi")
ap_lo_plot <- density_plot(cardio_train_0$ap_lo, "ap_lo")
...
grid.arrange(ap_hi_plot, ap_lo_plot)
```
...
cardio_train_cardio <- cardio_train_0 %>%
  filter(cardio == 1)
cardio_train_cardio %>% ggplot(aes(Age,ap_hi))+
  geom_point(shape = 20)+ scale_color_manual(values = c("blue","red"))+
  facet_grid(~gender)

cardio_train_cardio$ap_hi
```
...
```{r}
set.seed(2022)
split <- sample.split(cardio_train_0$cardio, SplitRatio=0.8)
train_cardio <- as.data.frame(cardio_train_0[split,])
test_cardio <- as.data.frame(cardio_train_0[!split,])
```
